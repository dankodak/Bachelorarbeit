...
if realSol(0.5423, 0.823) == 0
    if symmetric == 0
        A = lap_rbf(gamma, Xte(:,1), Xte(:,2), Xin(:,1).', Xin(:,2).',Xte(:,3), Xin(:,3).',Xte(:,4), Xin(:,4).');
    else
        A = lap2_rbf(gamma, Xte(:,1), Xte(:,2), Xin(:,1).', Xin(:,2).',Xte(:,3), Xin(:,3).',Xte(:,4), Xin(:,4).');
    end
    disc = A*alpha;
    real = f(Xte(:,1), Xte(:,2), Xte(:,3), Xte(:,4));
