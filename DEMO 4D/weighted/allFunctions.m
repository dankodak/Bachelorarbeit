...
syms a b c d x y k l gammas rbfs lap_rbfs ws prods fs realSols realSolPlots
%% PDE
switch pde
    case 'square'
%         ws(a,b)= -a^2 - b^2 + 2 -sqrt(a^4 - 2*a^2 + b^4 - 2*b^2 +2);
        ws(a,b,x,k) = sin(pi/2 *(a+1))*sin(pi/2 *(b+1))*sin(pi/2 *(x+1))*sin(pi/2 *(k+1));
%         ws(a,b)= 4 - sqrt(2*a^2 + 2) - sqrt(2*b^2+2) - sqrt( (2-sqrt(2*a^2 +2))^2 + (2-sqrt(2*b^2 +2))^2);
        fs(a,b,x,k) = 4*pi^2*sin(pi*a)*sin(pi*b)*sin(pi*x)*sin(pi*k);
%         realSols(a,b) = sin(pi*a)*sin(pi*b);
        realSols(a,b,x,k) = 0;
        realSolPlots(a,b,x,k) = sin(pi*a)*sin(pi*b)*sin(pi*x)*sin(pi*k);
...
switch kernel
    ...
    case 'gauss'
        switch symmetric
            case 0
                rbfs(gammas,a,b,c,d,x,y,k,l) = exp(-gammas*((c-a)^2 + (d-b)^2+ (y-x)^2+ (l-k)^2));
                rbf = matlabFunction(rbfs);
                prods(gammas,a,b,c,d,x,y,k,l) = rbfs(gammas,a,b,c,d,x,y,k,l) * w(a,b,x,k);
                lap_rbf = matlabFunction(-diff(prods,a,2) - diff(prods,b,2) - diff(prods,x,2) - diff(prods,k,2));
                lap2_rbf = 0;
            case 1
                rbfs(gammas,a,b,c,d,x,y,k,l) = ws(a,b,x,k) * ws(c,d,y,l) * exp(-gammas*((c-a)^2 + (d-b)^2+ (y-x)^2+ (l-k)^2));
                rbf = matlabFunction(rbfs);
                lap_rbfs = -diff(rbfs,c,2) - diff(rbfs,d,2) -diff(rbfs,y,2) - diff(rbfs,l,2);
                lap_rbf = matlabFunction(lap_rbfs);
                lap2_rbf = matlabFunction(-diff(lap_rbfs,a,2) - diff(lap_rbfs,b,2)-diff(lap_rbfs,x,2) - diff(lap_rbfs,k,2));
        end
end
end