function greedy(n, symmetric, kernel, pde, calc_error)
    ...
    for i = 1:n
        ...
        if calc_error == "abs"
            [error(i) , ~] = greedy_error(rbf, lap_rbf, lap2_rbf, w, f, gamma(i), alpha, Xin, Xbd, grideval, z, symmetric, 'abs');
        end
    ...
    if calc_error == "abs"
        [error(n+1) , ~] = greedy_error(rbf, lap_rbf, lap2_rbf, w, f, gamma(n+1), alpha, Xin, Xbd, grideval, z, symmetric, 'abs');
    else
        [error(n+1) , ~] = greedy_error(rbf, lap_rbf, lap2_rbf, w, f, gamma(n+1), alpha, Xin, Xbd, grideval, z, symmetric, 'res');
    end
    gamma = gamma(2:n+1);
    error = error(2:n+1);
    amount_points = (1 + size(Xbd, 1):n + size(Xbd, 1));
